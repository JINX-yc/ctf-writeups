...
Byte Lotus's "sister-property connectivity" tool shells out to ping with unsanitized user input, turning an SSRF-looking feature into straightforward OS command injection and yielding an initial web shell. From there, three loopback-only Flask/Gunicorn services were discovered: the public app, a Watchtower ops console, and a root-owned automation service. Watchtower's /api/config leaked FreePBX UCP credentials and revealed the automation /jobs/export endpoint. Exploiting the disclosed FreePBX auth bypass (CVE-2026-46376) required tunneling the loopback ports because the JS/AJAX-driven UCP login only works in a real browser. Inside UCP, a voicemail contained the automation bearer token, which unlocked /jobs/export—another command injection, this time executing as root.
